...
export function createStorageProvider(config: AppConfig): StorageProvider {
  switch (config.storageType) {
    case 'dropbox':
      // Use OAuth 2.0 when client credentials are present
      ...
      if (!config.dropboxAccessToken) {
        throw new Error('Dropbox configuration error: Either DROPBOX_ACCESS_TOKEN (legacy) or DROPBOX_CLIENT_ID + DROPBOX_CLIENT_SECRET (OAuth 2.0) is required');
      }
